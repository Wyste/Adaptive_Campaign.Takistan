...
Wyste          | 05/03/2015    | Handles dropping of intel items off enemies.
______________________________________________________________________________________________________________________*/
// RETURN: NOTHING
// USAGE : [_unit] call AIO_fnc_inteldrop
...
if (isServer || isDedicated) then {
  ...
  if (AIO_DEBUG) then {
    if (random 100 >= 1) then {
      ...
      [[_droppedItem,"Capture Intel","call AIO_fnc_intelpickup","red"],"AIO_fnc_addactionmp", false, true] spawn BIS_fnc_MP;
